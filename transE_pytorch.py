...
from transE import model
from transE.data import *
from transE.evaluation import *
from transE.utils import *
...
if __name__ == "__main__":
    ...
    argparser.add_argument('-d', '--dataset', type=str, default='FB13')
    ...
    trainTotal, trainList, trainDict = loadTriple('./transE/datasets/' + args.dataset, 'train2id.txt')
    validTotal, validList, validDict = loadTriple('./transE/datasets/' + args.dataset, 'valid2id.txt')
    tripleTotal, tripleList, tripleDict = loadTriple('./transE/datasets/' + args.dataset, 'triple2id.txt')
    config = Config()
    ...
    config.entity_total = getAnythingTotal('./transE/datasets/' + config.dataset, 'entity2id.txt')
    config.relation_total = getAnythingTotal('./transE/datasets/' + config.dataset, 'relation2id.txt')
    ...
    for epoch in range(config.train_times):
        ...
        if config.early_stopping_round > 0:
            if epoch == 0:
                ...
                hit10, best_meanrank = evaluation_transE(validList, tripleDict, ent_embeddings, rel_embeddings,
                                                         L1_flag, filter, config.batch_size,
                                                         num_processes=args.num_processes)
                agent.append(hit10Curve, epoch, hit10)
                agent.append(meanrankCurve, epoch, best_meanrank)
                torch.save(model, os.path.join('./transE/model/' + args.dataset, filename))
                best_epoch = 0
        ...
        # model.cuda()
            ...
            if not os.path.exists('./transE/model/' + args.dataset):
                os.makedirs('./transE/model/' + args.dataset)
            torch.save(model, os.path.join('./transE/model/' + args.dataset, filename))

    testTotal, testList, testDict = loadTriple('./transE/datasets/' + args.dataset, 'test2id.txt')
    oneToOneTotal, oneToOneList, oneToOneDict = loadTriple('./transE/datasets/' + args.dataset, 'one_to_one_test.txt')
    oneToManyTotal, oneToManyList, oneToManyDict = loadTriple('./transE/datasets/' + args.dataset,
                                                              'one_to_many_test.txt')
    manyToOneTotal, manyToOneList, manyToOneDict = loadTriple('./transE/datasets/' + args.dataset,
                                                              'many_to_one_test.txt')
    manyToManyTotal, manyToManyList, manyToManyDict = loadTriple('./transE/datasets/' + args.dataset,
                                                                 'many_to_many_test.txt')
    ...
    hit10Test, meanrankTest = evaluation_transE(testList, tripleDict, ent_embeddings, rel_embeddings, L1_flag, filter,
                                                head=0)

    hit10OneToOneHead, meanrankOneToOneHead = evaluation_transE(oneToOneList, tripleDict, ent_embeddings,
                                                                rel_embeddings, L1_flag, filter, head=1)
    hit10OneToManyHead, meanrankOneToManyHead = evaluation_transE(oneToManyList, tripleDict, ent_embeddings,
                                                                  rel_embeddings, L1_flag, filter, head=1)
    hit10ManyToOneHead, meanrankManyToOneHead = evaluation_transE(manyToOneList, tripleDict, ent_embeddings,
                                                                  rel_embeddings, L1_flag, filter, head=1)
    hit10ManyToManyHead, meanrankManyToManyHead = evaluation_transE(manyToManyList, tripleDict, ent_embeddings,
                                                                    rel_embeddings, L1_flag, filter, head=1)

    hit10OneToOneTail, meanrankOneToOneTail = evaluation_transE(oneToOneList, tripleDict, ent_embeddings,
                                                                rel_embeddings, L1_flag, filter, head=2)
    hit10OneToManyTail, meanrankOneToManyTail = evaluation_transE(oneToManyList, tripleDict, ent_embeddings,
                                                                  rel_embeddings, L1_flag, filter, head=2)
    hit10ManyToOneTail, meanrankManyToOneTail = evaluation_transE(manyToOneList, tripleDict, ent_embeddings,
                                                                  rel_embeddings, L1_flag, filter, head=2)
    hit10ManyToManyTail, meanrankManyToManyTail = evaluation_transE(manyToManyList, tripleDict, ent_embeddings,
                                                                    rel_embeddings, L1_flag, filter, head=2)
    ...
    if not os.path.exists('./transE/result/', args.dataset):
        os.makedirs('./transE/result/', args.dataset)
    with open(os.path.join('./transE/result/', args.dataset + '.txt'), 'a') as fw:
